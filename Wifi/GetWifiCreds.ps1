...
function Get-WiFiCreds {
    ...
    foreach ( $SSID in $WLAN ) {
	    ...
	    $AuthType = (($Network | Select-String "Authentication") -split(": "))[1] # set according to you language
        $Password = (($Network | select-string "Key Content") -split(": "))[1]	     # in us its Authentication
